...
def radon(f_vec, Ns, Nw, Nq):
    ...
    ws = W.flatten().reshape((-1, 1))
    ss = S.flatten().reshape((-1, 1))
    ...
    w_values = np.empty( (Np, Nq, 4) )
    s_values = np.empty( (Np, Nq, 4) )
    f_values = np.empty( (Np, Nq, 4, Ns) )
    p_values = np.empty( (Np, Nq, 4) )
    ...
    """
    print( 'p_values[:,:,0]', p_values[:,:,0].shape )
    print( 'poly_weights', poly_weights.shape )
    print( 'f_values[:,:,0]', f_values[:,:,0,:].shape )
    print( 's_values[:,:,0]', s_values[:,:,0].reshape((Np, -1, 1)).shape )
    print( 's', s.reshape((1, -1)).shape )
    print( 'poly_weights * f_values[:,:,0,:]', (poly_weights * f_values[:,:,0,:]).shape )
    print("p_values[:,:,0] = np.sum(poly_weights * f_values[:,:,0,:]/(s_values[:,:,0].reshape((Np,-1,1)) - s.reshape((1,-1)) + ff), axis=2)" )
    return 0
    """
    p_values[:,:,0] = np.sum(poly_weights * f_values[:,:,0,:]/(s_values[:,:,0].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2) / \
                      np.sum(poly_weights * 1/(s_values[:,:,0].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2)
    p_values[:,:,1] = np.sum(poly_weights * f_values[:,:,1,:]/(s_values[:,:,1].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2) / \
                      np.sum(poly_weights * 1/(s_values[:,:,1].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2)
    p_values[:,:,2] = np.sum(poly_weights * f_values[:,:,2,:]/(s_values[:,:,2].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2) / \
                      np.sum(poly_weights * 1/(s_values[:,:,2].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2)
    p_values[:,:,3] = np.sum(poly_weights * f_values[:,:,3,:]/(s_values[:,:,3].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2) / \
                      np.sum(poly_weights * 1/(s_values[:,:,3].reshape((Np,-1,1)) - s.reshape((1, 1,-1)) + ff), axis=2)
