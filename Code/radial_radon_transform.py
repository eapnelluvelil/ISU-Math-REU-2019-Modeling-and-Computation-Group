...
def radon(f_vec, Ns, Nw, Nq):
    ...
    for i in range(Nw):
        ...
        for j in range( Ns ):
            ...
            #plt.plot( x_quads[s_quads < 0], y_quads[s_quads < 0], 'r.' )

            w_values = np.empty( (Nq, 1) )
            s_values = np.empty( (Nq, 1) )
            f_values = np.empty( (Nq, 1, Ns) )
            p_values = np.empty( (Nq, 1) )

            s_values[:,0] = s_quads

            #print( f_values[:, 0, :Ns//2].shape )
            #print( f_values[:, 0, Ns//2:].shape )
            #f_values[:, 0, :Ns//2] = f_vec[i, :Ns//2]
            if j < Ns//2:
                f_values[:, 0, :Ns//2] = f_vec[i, :Ns//2]
                f_values[:, 0, Ns//2:] = np.flip(f_vec[i, :Ns//2])
            else:
                f_values[:, 0, :Ns//2] = np.flip(f_vec[i, Ns//2:])
                f_values[:, 0, Ns//2:] = f_vec[i, Ns//2:]
            ...
            #if j < Ns//2:
            f_hat[p] = r*np.dot( weights, p_values[:,0] )
            #else:
            #    f_hat[p] = f_hat[mp]
