...
def radon(fw, fs, Ns, Nw, Nq):
    ...
    x_quads = ss*np.cos(ws) - nodes.reshape((1, -1))*np.sin(ws)
    y_quads = ss*np.sin(ws) + nodes.reshape((1, -1))*np.cos(ws)
    ...
    dist[ dist > dw ] -= np.pi
    dist[ dist < -dw ] += np.pi
    #w_quads[np.abs( w_quads + np.pi ) < np.abs(w_quads)] += np.pi
    #"""
    ...
    f_hat = np.sum( weights.reshape((1, -1))*f_interp, axis=1 )
